...
def lid_is_closed_by_template(roi_bgr, ref_bgr, corr_thr=0.3):
    # template matching
...
def waffle_bottom_center(roi_bgr, v_dark_thr=85, min_ratio=0.08, circle_margin=0.92):
    hsv = cv2.cvtColor(roi_bgr, cv2.COLOR_BGR2HSV)
    v = hsv[:,:,2]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(WAFFLE_ROI_PATH): raise RuntimeError("waffle_roi_config.json not found")
    ...
    # 2) close refs（你現有程式怎麼讀就沿用；這裡用示意：refL/refR）
    #    你若原本是用 lid_close_refs.npz，就把讀取那段貼回來即可
    #    refL = ...
    #    refR = ...
    # 這裡先用 placeholder，避免你直接執行報錯：
    raise_if_missing = False
    if raise_if_missing:
        pass

    # !!! 你原本程式已有 refL/refR 的讀取，請保留原本那段 !!!
    # 我這裡用簡單方式：假設你工作目錄下有 close_ref_left.png / close_ref_right.png
    #refs = np.load("lid_close_refs.npz")
    #refL, refR = refs["refL"], refs["refR"]
    #thrL, thrR = float(refs["thrL"]), float(refs["thrR"])
    refL = cv2.imread("close_ref_left.png")
    refR = cv2.imread("close_ref_right.png")
    ...
    try:
        while True:
            ...
            L_closed, L_diff = lid_is_closed_by_template(roiL, refL, corr_thr=0.55)
            R_closed, R_diff = lid_is_closed_by_template(roiR, refR, corr_thr=0.55)
            L_open = not L_closed
            R_open = not R_closed
