...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not guardL:
            L_open = decide_open(L, black_thr, edge_thr, mode=decision_mode)
        else:
            L_open = (stable_state in ["ALL_OPEN","LEFT_OPEN"])
        if not guardR:
            R_open = decide_open(R, black_thr, edge_thr, mode=decision_mode)
        else:
        ...
        if len(buf)==vote_window:
            vals, cnt = np.unique(list(buf), return_counts=True)
            winner = vals[np.argmax(cnt)]
            if cnt.max() >= vote_need:
                stable_state = winner

        prevL, prevR = L, R

        vis = img.copy()
        draw_overlay(vis, cfg, L, R, L_open, R_open, stable_state, black_thr, edge_thr,
                     f"{os.path.basename(p)}  {idx+1}/{len(paths)}  (space next, b back, q quit)")
