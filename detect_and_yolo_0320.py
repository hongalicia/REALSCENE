...
class VisionServer:
    def __init__(self, shared_state: SharedState, host: str = "0.0.0.0", port: int = 9000, send_hz: float = 5.0): #設定Socket網路傳輸
        self.host = host
        self.port = port
...
def lid_is_closed_by_template(roi_bgr, ref_bgr, corr_thr=0.4):     #用template相似度判斷是不是關蓋
...
def state_from_lr(L_open, R_open):        #蓋子開關的狀態
...
def waffle_stuck_score(roi_bgr):         #用brown的顏色程度來判斷上蓋有沒有黏住鬆餅
...
def waffle_bottom_center(roi_bgr, v_dark_thr=85, min_ratio=0.08):    #計算鬆餅的中心位置
    hsv = cv2.cvtColor(roi_bgr, cv2.COLOR_BGR2HSV)
    v = hsv[:,:,2]
...
def transform_xyxy(x1: float, y1: float, x2: float, y2: float) -> Tuple[float, float, float, float]:     #座標轉換，將影像座標轉成Isaac-Sim的座標系統
    ...
    new_x1 = -49.1 + (y1 - 69)  * (1.245 / 10.006)
    new_y1 = -49.47 + (x1 - 268) * (1.221 / 10.002)
...
# Threads
# =========================================================
def camera_loop(framehub: FrameHub, stop_evt: threading.Event, cam_conf: dict):      #負責抓影像的thread
...
def lid_loop(framehub: FrameHub, shared_lid: SharedState, shared_yolo: SharedYOLO, stop_evt: threading.Event, conf: dict, debug_ui=True):   #檢查鬆餅機狀態的thread
    # 從config檔讀取相關參數
    cfg = conf["roi"]["lid"]
    ...
    while not stop_evt.is_set():
        ...
        if L_code == 3 and cL[0] is not None:
            _, _, wL, hL = cfg["left"]
            L_dx = float(cL[1] - hL / 2.0 - 1.0)     #計算鬆餅中心位置，目前有加上現場試測的調整參數
...
def yolo_loop(framehub: FrameHub, shared_yolo: SharedYOLO, stop_evt: threading.Event, conf: dict):    #使用YOLO偵測藍綠杯子的thread
    # 從config檔讀取相關參數
    yconf = conf["yolo"]
    ...
    host = str(yconf.get("host", "192.168.1.33"))
    ...
    if enable_send:    #如果config檔中有設定要傳送資料才去連線
        # commander = RobotCommander(host, port)
        commander2 = RobotCommander(host2, port2)

    print(f"[YOLO] model={model_path} conf={conf_th} imgsz={imgsz} -> send to {host}:{port} @ {send_hz}Hz")
    print(f"[YOLO] model={model_path} conf={conf_th} imgsz={imgsz} -> send to {host2}:{port2} @ {send_hz}Hz")

    # 設定門檻（pixel）只有超過這個門檻才會傳送物體位置
    MOVE_THRES_PX = 10.0
    ...
    while not stop_evt.is_set():
        ...
        # send class 0 and 1 line-by-line (same as your original)
        for class_id in (0, 1):
            ...
            if enable_send :
                ...
                if commander2 is not None:
                    commander2.send_goal(signal)
                time.sleep(0.05)
...
def zmq_stream_loop(framehub: FrameHub, shared_yolo: SharedYOLO, shared_lid: SharedState,
    ...
    while not stop_evt.is_set():
        ...
        # Get YOLO detections and lid state tuple
        yolo_dets = shared_yolo.get()
        lid_tuple = shared_lid.get()  # (L_code, L_dx, L_dy, R_code, R_dx, R_dy)
        L_code, L_dx, L_dy, R_code, R_dx, R_dy = lid_tuple

        # Derive open/close for display
        L_open = L_code in (2, 3, 4)
        R_open = R_code in (6, 7, 8)
        lid_state_str = state_from_lr(L_open, R_open)

        # Build annotated frame similar to UI
        vis = frame.copy()
        # Draw YOLO boxes
        for d in yolo_dets:
            x1, y1, x2, y2 = map(int, d["xyxy"])
        ...
        # Send annotated frame and JSON data
        try:
            ...
            # Avoid crashing the loop on transient send errors
            pass
